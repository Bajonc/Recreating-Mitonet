'''
Changes:
- Fixed a bug where the masks and images were misaligned (paths were not sorted)
'''
...
class _BaseDataset(Dataset):
    ...
    def __init__(
        ...
        for sd in self.subdirs:
            self.impaths_dict[sd] = sorted(glob(os.path.join(data_dir, f'{sd}/images/*')))
            self.mskpaths_dict[sd] = sorted(glob(os.path.join(data_dir, f'{sd}/masks/*')))
